...
// Verificar permiso para crear detalle de alquiler
verificarPermiso('crear');

// Obtener órdenes (con cliente y días) y maquinarias para los select
try {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $sql = "INSERT INTO detalle_alquiler (id_orden, id_maquinaria, dias_alquiler, subtotal) VALUES (?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id_orden, $id_maquinaria, $dias_alquiler, $subtotal]);
        echo "<script>alert('Detalle de alquiler registrado exitosamente'); window.location.href='R_detalle_alquiler.php';</script>";
    } catch (PDOException $e) {
        echo "Error: " . $e->getMessage();
    }
}
...
<head>
    ...
    <title>Registrar Detalle de Alquiler</title>
    <link rel="stylesheet" href="../public/style.css">
</head>
<body>
    <div class="container">
        <h2>Registrar Detalle de Alquiler</h2>
<form method="POST">
    <div class="form-group">
        ...
        <select id="id_orden" name="id_orden" required>
            ...
            <?php foreach ($ordenes as $orden): ?>
                <option value="<?= $orden['id_orden'] ?>" data-cliente="<?= htmlspecialchars($orden['nombre_cliente']) ?>" data-dias="<?= $orden['dias'] ?>">
    ...
    <div class="form-group">
        ...
        <select id="id_maquinaria" name="id_maquinaria" required>
            ...
            <?php foreach ($maquinarias as $maquinaria): ?>
                <option value="<?= $maquinaria['id_maquinaria'] ?>" data-precio="<?= $maquinaria['precio_diario'] ?>">
    ...
    <div class="form-group">
        ...
        <input type="text" min="1" name="dias_alquiler" id="dias_alquiler" required readonly>
    </div>
    <div class="form-group">
        <label for="subtotal">Subtotal ($):</label>
        <input type="text" step="0.01" name="subtotal" id="subtotal" required readonly>
    </div>
    <div class="form-group">
