...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$id_maquinaria || !$fecha || !$id_tecnico || $costo_mantenimiento === '' || !is_numeric($costo_mantenimiento) || $costo_mantenimiento < 0) {
        echo "<script>alert('Todos los campos son obligatorios y el costo debe ser un número mayor o igual a 0.'); window.history.back();</script>";
        exit;
    }

    try {
        $sql = "INSERT INTO mantenimiento (id_maquinaria, fecha, descripcion, costo_mantenimiento, id_tecnico) VALUES (?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id_maquinaria, $fecha, $descripcion, $costo_mantenimiento, $id_tecnico]);
        echo "<script>alert('Mantenimiento registrado exitosamente'); window.location.href='R_mantenimiento.php';</script>";
    } catch(PDOException $e) {
    ...
    <div class="container">
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <select name="id_maquinaria" id="id_maquinaria" required>
                    ...
                    <?php foreach ($maquinarias as $m): ?>
                        <option value="<?= $m['id_maquinaria'] ?>"><?= htmlspecialchars($m['nombre_maquinaria']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="costo_mantenimiento" id="costo_mantenimiento" required>
